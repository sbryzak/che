...
public class SqlScriptCreatorTest {
    ...
    public void createsScript() throws Exception {
        ...
        assertEquals(script.versionDir, "5.0.0-M7");
        assertEquals(script.resource.getLocation(), resource.getLocation());
        assertNull(script.version);
    ...
    @Test(expectedExceptions = FlywayException.class,
          expectedExceptionsMessageRegExp = "Sql script location must be either in " +
                                            "'location-root/versionDir' or in 'location-root/versionDir/provider-name', " +
                                            "but script 'schema/v1__init.sql' " +
                                            "is not in that kind of relation with root 'schema'")
